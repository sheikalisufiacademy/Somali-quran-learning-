...
export const Hero: React.FC<HeroProps> = ({ lang, onOpenRegister, onNavigate }) => {
  return (
    <section id="home" className="relative overflow-hidden bg-white text-slate-900 pt-8 pb-16 lg:pt-12 lg:pb-20 border-b border-slate-200">
      
      {/* Refined Geometric Background Accents (Anti-Glare, High Contrast) */}
      <div className="absolute top-0 right-0 w-80 sm:w-96 h-80 sm:h-96 bg-slate-100 rounded-bl-[100px] pointer-events-none -z-0 opacity-70" />
      <div className="absolute bottom-0 left-0 w-64 sm:w-80 h-64 sm:h-80 bg-orange-50/50 rounded-tr-[100px] pointer-events-none -z-0" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
        ...
        >
          <div className="inline-block px-6 py-1.5 rounded-full bg-slate-100 border border-slate-300 shadow-xs">
            <span className="font-quran text-lg md:text-xl text-[#0B192C] tracking-wide font-normal">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-8 items-center">
          ...
          >
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-[2.85rem] font-black tracking-tight leading-[1.18] text-[#0B192C]">
              {lang === 'so' ? (
                <>
                  Baro <span className="text-[#0B192C] underline decoration-orange-500 decoration-4 underline-offset-6">Qur’aanka Kariimka</span>, Barashada Tajwiidka & Higaada Online
                </>
              ) : (
                <>
                  Learn the <span className="text-[#0B192C] underline decoration-orange-500 decoration-4 underline-offset-6">Holy Quran</span>, Tajweed & Noorani Qaida Online
                </>
              )}
            </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-left max-w-xl mx-auto lg:mx-0">
              <div className="flex items-center gap-2.5 text-sm font-bold text-slate-900 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-3">
              ...
              {onNavigate ? (
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-4 text-base font-bold text-[#0B192C] bg-white hover:bg-slate-100 border-2 border-[#0B192C] rounded-xl transition-all flex items-center justify-center gap-2 shadow-xs cursor-pointer"
                >
                ...
                <a
                  href="#courses"
                  className="w-full sm:w-auto px-6 py-4 text-base font-bold text-[#0B192C] bg-white hover:bg-slate-100 border-2 border-[#0B192C] rounded-xl transition-all flex items-center justify-center gap-2 shadow-xs"
                >
                  <GraduationCap className="w-5 h-5 text-orange-500" />
                  <span>{lang === 'so' ? 'Daawo Koorsooyinka & Manhajka' : 'Explore Courses'}</span>
                </a>
            ...
            {/* Parent Trust Rating */}
            <div className="flex items-center justify-center lg:justify-start gap-4 pt-2 text-xs text-slate-600">
              ...
              <div>
                <span className="font-black text-[#0B192C]">4.98 / 5.0</span>
          ...
          >
            ...
            <div className="relative rounded-3xl bg-white border-2 border-[#0B192C] p-6 sm:p-7 shadow-2xl overflow-hidden">
              
              {/* Header inside card */}
              <div className="flex items-center justify-between pb-4 border-b border-slate-200">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#0B192C] flex items-center justify-center text-white">
                    <Video className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="font-black text-sm text-[#0B192C]">
                ...
                <span className="px-3 py-1 text-[11px] font-black rounded-full bg-[#0B192C] text-white">
                  {lang === 'so' ? '100% BILAASH' : '100% FREE'}
                </span>
              </div>

              {/* Sample Quran Page / Noorani Preview Inside Card */}
              <div className="my-4 p-4 rounded-2xl bg-slate-50 border border-slate-200 text-center space-y-2">
                ...
                <div className="font-quran text-2xl sm:text-3xl text-[#0B192C] py-1 leading-relaxed font-bold">
              ...
              <div className="space-y-2 text-xs text-slate-800 mb-5">
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                  <span className="flex items-center gap-2 font-bold text-slate-700">
                    <ShieldCheck className="w-4 h-4 text-orange-500" />
                    {lang === 'so' ? 'Xulashada Macallinka:' : 'Teacher Preference:'}
                  </span>
                  <span className="font-black text-[#0B192C]">{lang === 'so' ? 'Lab ama Dheddig' : 'Male or Female'}</span>
                </div>
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                  ...
                  <span className="font-black text-[#0B192C]">{lang === 'so' ? '30 Daqiiqo (Qiimeyn)' : '30 Mins Live Evaluation'}</span>
                </div>
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                  ...
                  <span className="font-black text-[#0B192C]">1-on-1 Zoom / Google Meet</span>
                </div>
              </div>
              ...
              <div className="mt-3 text-center">
                <a
                  ...
                  className="inline-flex items-center gap-1.5 text-xs font-black text-slate-800 hover:text-orange-600 transition-colors"
                >
